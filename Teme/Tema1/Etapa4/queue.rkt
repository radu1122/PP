...
(define (enqueue x q)
  (if (< (queue-size-l q) (+ (queue-size-r q) 1))
      (struct-copy queue q [left (rotate (queue-left q) (append (list x) (queue-right q)) empty-stream)] [right '()]  [size-r 0] [size-l (+ (+ (queue-size-l q) (queue-size-r q)) 1)] )
...
; Veți întoarce coada actualizată.s
; Atenție: în urma înlăturării unui element, poate fi necesară o rotație!
(define (dequeue q)
  (if (< (- (queue-size-l q) 1) (queue-size-r q))
      (struct-copy queue q [left (rotate (stream-rest (queue-left q)) (queue-right q) empty-stream)] [right '()]  [size-r 0] [size-l (- (+ (queue-size-l q) (queue-size-r q)) 1)] )
